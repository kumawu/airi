...
export const getUserFortune = async (token: string, locale:string='en-US') => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/users/user/fortune?locale=${locale}`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const getUserWallet = async (token: string) => {
	...
	const res = await fetch(`${WEBUI_API_BASE_URL}/users/user/wallet`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
